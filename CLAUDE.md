...
cp plugin/stash-sense-core.js plugin/stash-sense.js plugin/stash-sense-operations.js \
   ...
   <stash-plugin-repo-checkout>/plugins/stash-sense2/
cp plugin/stash-sense.yml <stash-plugin-repo-checkout>/plugins/stash-sense2/stash-sense2.yml
...
cd <stash-plugin-repo-checkout> && ./build.sh
```
...
**Layers 1/2 also carry `timestamp_sec` per cached face** (`scene_face_embeddings.timestamp_sec`) — real seconds-into-the-video for that specific detected face, used only for Face Recommendations' scene-player "jump to frame" buttons (`top_timestamps_sec` on a `PerformerMatchResponse`/scene_face_match candidate). Two gotchas already hit in production, both now fixed:
  - `matching_mode="hybrid"` (the mode every bulk/cached identify actually uses — see `_cluster_and_match()`'s dispatch in `identification_router.py`) silently dropped `frame_timestamps` entirely until `scene_matcher.hybrid_matching()` was fixed to thread it through its own internal cluster component. `matching_mode="frequency"` still never resolves timestamps (no per-frame-cluster concept to resolve them from) — expected, not a bug.
  - Every sprite-tile face on a scene used to share one literal `frame_index=-2` sentinel (`_process_sprite_frames()`), so only one sprite timestamp per scene was ever resolvable even though each row's own `timestamp_sec` was stored correctly. Fixed to assign each sprite face its own unique negative `frame_index` going forward. Already-cached data before this fix needed a real backfill (not just new-code-goes-forward): sprite rows are fixed by schema migration v18 (`recommendations_db.py`, pure local SQL, runs automatically at startup); video-frame rows (which never had `timestamp_sec` written at all pre-fix) are fixed by `backfill_frame_timestamps.run_video_timestamp_backfill_once()`, launched as a one-time non-blocking background task from `main.py`'s startup (gated by a `user_settings` flag) — reconstructs each row's real timestamp from `scene_signal_cache`'s stored `num_frames`/`start_offset_pct`/`end_offset_pct` plus one cheap Stash GraphQL call per scene for duration, **not** a re-fingerprint (no ffmpeg, no GPU). `backfill_frame_timestamps.py` also works as a standalone CLI script (`--dry-run` supported) for an immediate manual run instead of waiting for the next restart.
...
A second, much smaller usearch index built from *this Stash instance's own* performer cover images (`local_performer_index.py`), queried alongside the main `performers.db` index during identification and merged into the same result list. Local candidates get their `combined_distance` multiplied by `LOCAL_MATCH_BOOST` (currently `0.85`, in `matching.py`) before merging — a face in the user's own library is more likely to be a performer they've already added than a random main-DB entry. `fuse_local_results()` builds these candidates (flat boost, no cross-model logic — buffalo_l is a single model, so there's nothing to arbitrate between; the legacy dual FaceNet512+ArcFace agreement-gating this used to need is gone along with that pipeline), and `merge_local_candidates()` combines them with the main index's own results. Two independent duplicate-detection passes inside `merge_local_candidates()`: (1) a local performer's linked `stashdb_id` matching a main-index candidate's own universal_id (works only for a real stashbox candidate the local performer is already stash_id-linked to), and (2) a local performer's stored `urls` matching a *catalogue* (seekfans/pornbox) candidate's `profile_url`/`catalogue_url` (`stashbox_utils.normalize_url_for_compare`) — catches a same-person duplicate stash_id-linking can't, since catalogue sources have no real stash_id at all. Real-stashbox candidates aren't URL-compared (their urls aren't in `performers.json` at all, and fetching them live per candidate on every match would be too expensive for this hot path) — a local performer not yet stash_id-linked to a real stashbox match still shows as two separate candidates until linked (see `recommendations_router.py`'s identity-resolution flow below, which writes that link when the user resolves an ambiguous accept). Separately, `GET /recommendations/{rec_id}` for a `scene_face_match` recommendation does a live, on-demand `stash_id` lookup (`_enrich_scene_face_match_candidates_with_local_links`) to fill in `local_performer_id` for a real-stashbox candidate the vector-based merge above missed entirely — e.g. a local performer with no (or no *matchable*) cover photo, so they never had an embedding in the local index to merge against in the first place. Scoped to the single recommendation being viewed, not the whole pending list, to keep the per-candidate `findPerformers` query cheap.
