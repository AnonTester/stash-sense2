...
def db_reader():
    ...
    conn.executescript("""
        ...
        INSERT INTO performers (id, canonical_name, face_count) VALUES (1, 'Performer With Faces', 15);
        INSERT INTO performers (id, canonical_name, face_count) VALUES (2, 'Performer Fewer Faces', 10);
        INSERT INTO performers (id, canonical_name, face_count) VALUES (3, 'Performer No Coverage', 0);

        -- Insert stashbox IDs
        INSERT INTO stashbox_ids VALUES (1, 'stashdb.org', 'abc-123');
        INSERT INTO stashbox_ids VALUES (2, 'stashdb.org', 'fewer-faces');
        INSERT INTO stashbox_ids VALUES (3, 'stashdb.org', 'no-coverage');
    """)
    conn.close()
...
class TestDatabaseReaderBenchmarkMethods:
    ...
    def test_get_face_count_for_performer_different_counts(self, db_reader):
        ...
        count1 = db_reader.get_face_count_for_performer("stashdb.org:fewer-faces")
        count2 = db_reader.get_face_count_for_performer("stashdb.org:no-coverage")
        assert count1 == 10
        assert count2 == 0
